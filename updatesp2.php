...
Welcome To Marks Calculation Page
...
echo "<script>alert('Datae Base Updated Successful!'); window.location.href('marks.php');</script>";
 ...
 echo  $statusf="<meta http-equiv='refresh' content='0; url=rankm2.php'>"; 
 }
?>
<?php } ?>			<?php
	  /******************** ERROR MESSAGES*************************************************
...
$qry = mysql_query("select sum(((c101))*coef) as total
...
from resit where first_name='$first_name' and classes='$classes' and form='$take' and c101>''  and c105='2'  "); 
$row = mysql_fetch_assoc($qry); 
echo $saves=$row['total']; 
 ...
 <input type='text' name='fre2[]' value='<?php

printf("%01.2f", $ash=($save/$saves)); 
 ...
 <input type='text' name='c106[]' value='<?php printf("%01.0f", $ash=($save/$saves)*(100));;?>' size='8' style='BACKGROUND:#CCC;' maxlength='2'>
 </DIV>
