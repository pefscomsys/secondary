...
<?php
	...
	$query = "SELECT * FROM `teach` WHERE `class` = '$class' ";
    ...
    list($year) = mysqli_fetch_array($ress);
	?>
			...
			<table class="table table-bordered">
				...
				else
				{
					...
					while($row = mysqli_fetch_array($result))
					{
						...
						//now get the teacher's full names
						$query = "SELECT `full_name` FROM `users` WHERE `user_name`  = '$username' ";
						$res = mysqli_query($dbc, $query);

						list($teacher) = mysqli_fetch_array($res);

                        //set values for the various terms.
                        $first_term = FALSE;
                        $second_term = FALSE;
                        $third_term = FALSE;

                        //check if the marks have been filled.
                        //loop through all the terms and check for this subject
                        for ($term=1; $term < 4 ; $term++) {
                            /// check if the marks are there.
                            $query = "SELECT * FROM `resit` WHERE `subject` = '$subject_code'
                                AND `c105` = '$term' AND `form` = '$year' ";
                            $term_result = mysqli_query($dbc, $query);
