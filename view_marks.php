...
Track Activities
  ...
  <?php
	...
	//now check if marks have been filled for that subject and year
	$query = "SELECT * FROM `resit` WHERE `c105` = '$term' AND `form` = '$year'
				AND `subject` = '$subject' AND `classes` = '$class' ";
	...
	else
	{
		...
		/// if there are marks. then get the class list
        ...
        <div class="table-responsive">
			<table class="table table-stripped table-bordered">
				...
				while($row = mysqli_fetch_array($result))
				{
					...
					$name = mysqli_real_escape_string($dbc, $row['first_name']);
					
					//now get the student's marks. 
					$query = "SELECT * FROM `resit` WHERE `first_name` = '$name' AND `c105` = '$term'
								AND `form` = '$year' AND `subject` = '$subject' AND `classes` = '$class' ";
					$res = mysqli_query($dbc, $query)
						or die("Could not get marks");
						
					if(mysqli_num_rows($res) == 0)
					{
						//the studetns marks have not been entered. 
						$quest = true;
					}
					else
					{
