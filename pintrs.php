...
<tr class="listheader">
<td></td>
<td>Enter Search Keyword</td><td><input type="text" name="search" size="39"placeholder="name #"style="float:left; width:200px; height:38px;background:#fff;" /></td>
...
$search=$_POST["search"];
$flag=0;
$query="select * from balancesheet where  db1='$take' and id='1' or id='2' or id='3' and  status>''  and status like '%$search%' LIMIT 20";
$result=mysql_query($query);
$i=0;
 while ($row = mysql_fetch_array($result)) {
if($i%2==0)
...
<td style="float:left; height:25px;"><?php echo $mats=$row["fname"]; $css=$row["status"];$X=$row["xclass"]; $cats=$row["name"];$roll=$row["recid"];$page=$row["page"];?></td>
<td>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $css;
$qry = mysql_query("select fname as total
from courses where matricule='$mats' and  db1='$take'  "); 
$row = mysql_fetch_assoc($qry); 
 $savee4=$row['total'];  ?></td>
<td>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $cats; ?>   <?php echo $X; ?>   </td>

<td>&nbsp;&nbsp;&nbsp;&nbsp;<a href="dellres.php?roll=<?php echo $roll; ?>">Delete Receipts</a></td>
<td>
<?php 
