...
Track Activities
  ...
  width: 40px;
  height: 25px;color:#50a3a2;">
  <div style="float:left;margin-top:8px;font-size:19px;font-weight:bold;">
Debtors List
  </div>
  </div>
...
<?php
$rs=mysql_query("Select * from classes order by  class ASC",$link);
...
<select name="department">
<?php
$rs=mysql_query("Select * from levels order by  cat ASC",$link);
	  while($row=mysql_fetch_array($rs))
{
if($row[1]==$fees)
{
