...
if(!empty($_POST["submit"]))
{
 foreach($_POST["id"] as $roll)
 {
  ...
  mysql_query("update marks SET
               teacherreligion='".$_POST['c103'][$i]."',
			   
               middle_name='".$_POST['name11'][$i]."',
...
echo "<script>alert('Datae Base Updated Successful!'); window.location.href('marks.php');</script>";
 ...
 echo  $statusf="<meta http-equiv='refresh' content='0; url=rankf2.php?roll=$clasf&term=2&year=$takess'>";  }
?>
<?php } ?>			<?php
	  /******************** ERROR MESSAGES*************************************************
...
from resit where first_name='$first_name' and classes='$classes' and form='$take' and c105='2'  "); 
...
echo $save=$row['total']; 
 ...
 <input type='text' name='c102[]' value='<?php
  ...
  "select sum(coef) as total
from resit where first_name='$first_name' and classes='$classes' and form='$take' and (
c101
AND c102
) <> ''  and c105='2' 
...
FROM `resit`
WHERE c105 = '2'
AND first_name='$first_name' and classes='$classes' and form='$take'
AND (
c101
OR c102
) > ''
group by  first_name,classes,form,c105,coef
...
$status="0";
 ...
 <input type='text' name='name11[]' value="<?php printf("%01.0f", ($status*100));?>" size='8' style='BACKGROUND:#CCC;' maxlength='2'>
 </DIV>
