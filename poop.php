...
$search=$_POST["search"];
$flag=0;
$query="select * from employer where  fname like '%$search%'    AND status='' order by matricule ASC ";
$result=mysql_query($query);
$i=0;
 while ($row = mysql_fetch_array($result)) {
if($i%2==0)
...
<td><input type="checkbox" name="users[]" value="<?php echo $row["recid"]; ?>" ></td>
<td><?php echo $mxx=$row["matricule"]; ?></td>
<td><?php echo $row["fname"]; ?></td>
<td><?php echo $row["contact1"]; ?>/ <?php echo $row["contact2"]; ?></td>
<td>
<a href="editma.php?roll=<?php echo $row["recid"]; ?>">Edit Records</a> | <a href="delle.php?roll=<?php echo $row["recid"]; ?>">Delete Records</a> </td>
...
$row = mysql_fetch_assoc($qry); 
 $savep4=$row['total']; 
				?>
										<img src="load_images.php?pic_id=<?php echo $savep4;?>" width="30px" height="25p" style="border:1px solid #000;box-shadow:1px 1px 1px #fff;">
</div></td>
